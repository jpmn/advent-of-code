const input = `aczupnetwp-mfyyj-opalcexpye-977[peyac]
...
const answer1 = ((data) => {
  const rooms = data.map(item => {
    let matches = item.match(/([a-z\-]*)\-(\d*)\[(\w*)\]/);
    return {
    ...
    let count = Array.from(room.name).reduce((d, x) => {
      d[x] = d[x] ? d[x] + 1 : 1;
      return d;
    }, {});
    
    let checksum = Object.keys(count).map(k => {
      return {
        key: k, 
        value: count[k]
      };
...
console.log('Day 4, Answer 1:', answer1);
...
const answer2 = ((data) => {
  ...
  const room = rooms.find(room => {
    let real = Array.from(room.name).reduce((r, x) => {
      return r + alphabet[(alphabet.indexOf(x) + room.id) % alphabet.length];
    }, '');
    return real.includes('north');
  });
  
  return room.id;
})(input.split(`\n`));

console.log('Day 4, Answer 2:', answer2);
